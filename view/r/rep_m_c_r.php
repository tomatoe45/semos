        ...
        <div class="page-title animate fadeInUp">
            ...
            <div class="row">
              ...
              <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                  <div class="x_title">
                    <h2>MAINTENANCE CONCERN REPORT </h2>
                    <div class="clearfix"></div>
                  </div>
                   <?php 
                    global $database;
                    ...
                    $stmt = $database->prepare("SELECT * FROM " . TBL_COMPLAINT . " WHERE cat_comp_id = '3' AND status BETWEEN '2' AND '5' ");
                    $stmt->execute(array(':start' => $start. '%'));
                    $g_allMC = $stmt->fetchAll(PDO::FETCH_ASSOC);
                    
                    foreach ($g_allMC as $mc) {
                      $time_one = new DateTime($mc['req_date']);
                      $time_two = new DateTime($mc['res_date']);
                      $difference = $time_one->diff($time_two);
                      $tableContent = $tableContent . "<tr>"
                                                      . "<td>" . $mc['comp_id'] . "</td>"
                                                      . "<td>". $mc['ticket'] ."</td>"
                                                      . "<td>". $mc['subject'] ."</td>"
                                                      . "<td>". date('M. d Y', strtotime($mc['req_date'])) . "<br>" . date('h:i A', strtotime($mc['req_date'])) ."</td>"
                                                      . "<td>". ( $mc['res_date'] != '0000-00-00 00:00:00' ? date('M. d Y', strtotime($mc['res_date'])) . "<br>" . date('h:i A', strtotime($mc['res_date'])) : '&hellip;') ."</td>"
                                                      . "<td>". ($mc['res_date'] != '0000-00-00 00:00:00' ? $difference->format('%h hrs <br> %i minutes') : '...') ."</td>"
                                                      . "<td>". ucwords($mc['assisted']) ."</td>"
                                                      . "<td>". ($mc['status'] == 5 ? "<i class='fa fa-check-square-o'></i> RESOLVED"
                                                                                    : ($mc['status'] == 2 ? "<i class='fa fa-exclamation-triangle'></i> PENDING"
                        ...
                        <div class="col-md-4">
                              <div class="input-group">
                                ...
                                <div class="input-group-btn">
                                  ...
                                  <a href="rep_m_c_r.php" class="btn btn-primary"><i class="fa fa-refresh"></i></a>
                                </div>
                              </div>
                          </form>
                        </div>
